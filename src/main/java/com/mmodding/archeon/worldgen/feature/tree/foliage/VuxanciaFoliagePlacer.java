...
import com.mmodding.library.math.api.function.MathFunction;
...
public class VuxanciaFoliagePlacer extends FoliagePlacer {
	...
	public VuxanciaFoliagePlacer(IntProvider radius, IntProvider offset) {
		super(radius, offset);
	}

	@Override
	public FoliagePlacerType<?> getType() {
		return ArcheonTreeParts.VUXANCIA_FOLIAGE_PLACER;
	}
	...
	private void placePatch(TestableWorld world, BlockPlacer replacer, Random random, TreeFeatureConfig config, BlockPos centerPos, boolean isBig, Set<BlockPos> collector) {
		...
		collector.forEach(pos -> {
			int validNeighbors = Direction.stream().filter(
				direction -> world.testBlockState(
					...
					state -> state.isOf(config.foliageProvider.get(random, pos).getBlock()) || state.isOf(ArcheonBlocks.VUXANCIA_LOG)
				)
			...
			if (validNeighbors <= 4 || (validNeighbors == 5 && world.testBlockState(pos.up(), BlockState::isAir))) {
				...
				double cancellationProbability = function.apply(Vec3d.of(pos.subtract(centerPos.withY(pos.getY()))).length());
				if (random.nextDouble() >= cancellationProbability) {
					removed.add(pos);
				}
			}
		});
		removed.forEach(pos -> replacer.placeBlock(pos, Blocks.AIR.getDefaultState()));
		collector.removeIf(removed::contains);
	}

	private void placeCircle(TestableWorld world, BlockPlacer replacer, Random random, TreeFeatureConfig config, BlockPos centerPos, int size, Set<BlockPos> collector) {
	...
	protected void placeVuxanciaFoliage(TestableWorld world, BlockPlacer replacer, Random random, TreeFeatureConfig config, BlockPos centerPos, BlockPos pos, Set<BlockPos> collector) {
