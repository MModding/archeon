...
public class HollowCypressLogFeature extends Feature<HollowCypressLogFeature.Config> {
			...
			if (randomPickedLength * 0.66 <= numberOfBlocksNotOnAir) { // Simple check that at least two thirds of block positions are not on air blocks
				for (BlockPos pos : blockPositions) {
					context.getWorld().setBlockState(
						pos,
						context.getConfig().hollowLog().get(context.getRandom(), pos).with(Properties.AXIS, randomPickedDirection.getAxis()),
